...
int img_read_callback(glc_thread_state_t *state)
{
	...
	if (state->header.type == GLC_MESSAGE_CTX) {
		...
		if (img->prev_pic)
			img->prev_pic = (char *) realloc(img->prev_pic, img->w * img->h * 3);
		else
			img->prev_pic = (char *) malloc(img->w * img->h * 3);
		memset(img->prev_pic, 0, img->w * img->h * 3);
...
int write_pic(struct img_private_s *img, char *pic, unsigned int w, unsigned int h, int num)
{
	...
	if ((w * 3) % 4 != 0) {
		for (i = 0; i < h; i++) {
			fwrite(&pic[(i * w) * 3], 1, w * 3, fd);
			fwrite("\x00\x00\x00\x00", 1, 4 - ((w * 3) % 4), fd);
		}
	} else
		fwrite(pic, 1, w * h * 3, fd);
